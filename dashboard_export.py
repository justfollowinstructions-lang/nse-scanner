...
def export_html_dashboard(
    ...
    for key, row in merged.items():
        try:
            ...
            log.debug(f"dashboard_export: skipped {key} due to error: {e}")
...
def _collect_all_signals(df_today: pd.DataFrame, con, scan_date: str, log) -> tuple[dict, dict]:
    ...
    if df_today is not None and len(df_today):
        for _, r in df_today.iterrows():
            ...
            _add(row, "today")
            for tag_key, cat_key in _TAG_TO_CATEGORY.items():
                if tag_key in str(row.get("tags") or "").split(","):
                    _add(row, cat_key)
...
def _build_symbol_card(row: dict, read_cache_fn, ohlcv_cache: dict) -> Optional[dict]:
    ...
    daily = ohlcv_cache.get(stock)
    if daily is None:
        daily = read_cache_fn(stock, "1d", 400)
        ohlcv_cache[stock] = daily
    if daily is None or daily.empty:
        return None

    timeframes_json = _build_timeframes_json(stock, daily, read_cache_fn)
...
def _resample(daily: pd.DataFrame, rule: str) -> Optional[pd.DataFrame]:
    """Fallback resample from daily bars if the real weekly/monthly cache
    (populated by data_updater.py) is missing for this stock."""
    if daily is None or daily.empty:
        return None
    try:
        ...
        v = daily["Volume"].resample(rule).sum()
        out = pd.DataFrame({"Open": o, "High": h, "Low": l, "Close": c, "Volume": v}).dropna()
        return out
    except Exception:
        try:
            # older pandas without "ME" alias
            return _resample(daily, "M" if rule == "ME" else rule)
        except Exception:
            return None
